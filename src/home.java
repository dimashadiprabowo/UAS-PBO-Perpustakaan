...
public class home extends javax.swing.JFrame {
        ...
        if (baris >= 0) {
            ...
            if (x == JOptionPane.YES_OPTION) {
                try {
                    ...
                    String sql = "DELETE FROM peminjaman WHERE id_anggota= '"+id_anggota+"' AND kode = '"+kode+"' AND tanggal = '"+tanggal+"'";
    ...
    if (baris >= 0) {
        String id_anggota = tbl4.getValueAt(baris, 0).toString();   // kolom id_anggota
        String nama = tbl4.getValueAt(baris, 1).toString();         // kolom nama
        String tanggal = tbl4.getValueAt(baris, 3).toString();      // kolom tanggal pinjam
        ...
        if (x == JOptionPane.YES_OPTION) {
            try {
                ...
                // cari id_peminjaman dulu berdasarkan data peminjaman
                ...
                if (rs.next()) {
                    ...
                    String hapusSQL = "DELETE FROM pengembalian WHERE id_peminjaman = ?";
                    PreparedStatement pstHapus = con.prepareStatement(hapusSQL);
                    pstHapus.setString(1, id_peminjaman);
                    ...
                    tampil_pengembalian(); // refresh data
                } else {
...
private void ubah4_valueMouseClicked(java.awt.event.MouseEvent evt) {                                         
    int baris = tb_pengembalian.getSelectedRow();
    if (baris >= 0) {
        // Ambil nilai dari tabel tampilan (yang sumbernya gabungan)
        String id_anggota = tbl4.getValueAt(baris, 0).toString(); // pastikan ini kolom id_anggota
        String nama = tbl4.getValueAt(baris, 1).toString();       // pastikan ini kolom nama
        String tanggal = tbl4.getValueAt(baris, 3).toString();    // pastikan ini kolom tanggal pinjam
